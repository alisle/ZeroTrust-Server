...
public interface  NetworkRepository extends JpaRepository<Network, UUID> {
    ...
    @Query(value = "SELECT new com.zerotrust.model.AgentCount((SELECT agent.name FROM ViewAgent agent WHERE agent.id = connection.destinationAgent.id) as agent,  connection.destinationAgent.id as uuid, COUNT(*) as count) FROM ViewConnectionLink connection WHERE connection.alive = TRUE AND connection.destinationNetwork.id = ?1 GROUP BY connection.destinationAgent")
    List<AgentCount> activeDestinationConnections(@Param("network_id") UUID network);


    @RestResource(exported = false)
    @Query(value = "SELECT new com.zerotrust.model.AgentCount((SELECT agent.name FROM ViewAgent agent WHERE agent.id = connection.sourceAgent.id) as agent,  connection.sourceAgent.id as uuid, COUNT(*) as count) FROM ViewConnectionLink connection WHERE connection.alive = TRUE AND connection.sourceNetwork.id = ?1 GROUP BY connection.sourceAgent")

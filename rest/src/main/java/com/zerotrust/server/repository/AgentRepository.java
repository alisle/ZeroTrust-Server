...
import com.zerotrust.server.model.ProcessCount;
import com.zerotrust.server.model.UserCount;
...
public interface AgentRepository extends JpaRepository<Agent, UUID> {
    ...
    @RestResource(exported = false)
    @Query("SELECT new com.zerotrust.server.model.UserCount(sourceUserName, COUNT(*)) FROM ConnectionLink WHERE sourceAgent.id = ?1 GROUP BY sourceUserName")
    List<UserCount> countSourceUsername(@Param("source_agent_id") UUID id);


    @RestResource(exported = false)
    @Query("SELECT new com.zerotrust.server.model.UserCount(destinationUserName, COUNT(*)) FROM ConnectionLink WHERE destinationAgent.id = ?1 GROUP BY destinationUserName")
    List<UserCount> countDestinationUsername(@Param("destination_agent_id") UUID id);

    @RestResource(exported = false)
    @Query("SELECT new com.zerotrust.server.model.UserCount(username, COUNT(*)) FROM Connection WHERE agent.id = ?1 GROUP BY username")
    List<UserCount> countUsername(@Param("agent_id") UUID id);


    @RestResource(exported = false)
    @Query("SELECT new com.zerotrust.server.model.ProcessCount(sourceProcessName, COUNT(*)) FROM ConnectionLink WHERE sourceAgent.id = ?1 GROUP BY sourceProcessName")
    List<ProcessCount> countSourceProcess(@Param("source_agent_id") UUID id);

    @RestResource(exported = false)
    @Query(value = "SELECT new com.zerotrust.server.model.ProcessCount(destinationProcessName, COUNT(*)) FROM ConnectionLink WHERE destinationAgent.id = ?1 GROUP BY destinationProcessName")
    ...
    @Query(value = "SELECT new com.zerotrust.server.model.AgentCount((SELECT agent.name FROM Agent agent WHERE agent.id = connection.destinationAgent.id) as agent,  connection.destinationAgent.id as uuid, COUNT(*) as count) FROM ConnectionLink connection GROUP BY connection.destinationAgent")
    ...
    @Query(value = "SELECT new com.zerotrust.server.model.AgentCount((SELECT agent.name FROM Agent agent WHERE agent.id = connection.sourceAgent.id) as agent,  connection.sourceAgent.id as uuid, COUNT(*) as count) FROM ConnectionLink connection GROUP BY connection.sourceAgent")
